...
local t = {
storage = 633111,  -- storage, so mude se tiver usando pra outra coisa.
tempo = 5, -- Tempo em horas.
}

local function greetCallback(cid)
	local msg = 'Olá '.. getCreatureName(cid) .."o que deseja ? {Batalhar}, {Ajudar}"
...
function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, 'sim') and talkState[talkUser] == 1) then
		if getPlayerStorageValue(cid, t.storage) < os.time()  then
			...
			setPlayerStorageValue(cid, t.storage, os.time() + t.tempo * 60 )
				...
				doPlayerPopupFYI(cid, "[ Mizuki ] estou descansando, espere ".. getPlayerStorageValue(cid, t.storage) - os.time() .." segundos para batalhar comigo novamente.")
	end
	talkState[talkUser] = 0

	elseif(msgcontains(msg, 'ajudar')) then
	selfSay('Roube o pergaminho secreto que está guardado em um baú proximo ao Hokage Naruto. {entregar}', cid)
	talkState[talkUser] = 2
	elseif(msgcontains(msg, 'entregar') and talkState[talkUser] == 2) then
	if getPlayerLevel(cid) >= 5 then
	if(getPlayerItemCount(cid, 2105) >= 1 and getPlayerStorageValue(cid, 12001) < 1 ) then
	doPlayerRemoveItem(cid, 2105, 1)
	doPlayerAddItem(cid, 2399, 50)
	...
	doPlayerSendTextMessage(cid,22,"Você ganhou uma recompensa.")
	else
	selfSay('Já trouxe meu pergaminho ? Então não tenho mais nada para você.', cid)
	end
	else
	selfSay('Você precisa de level 5 ou mais para realizar essa missão.', cid)
	end
	talkState[talkUser] = 0
