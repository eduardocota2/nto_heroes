...
local nome = "Tobi" -- nome do monstro que vai parecer

local t = {
storage = 671010,  -- storage, so mude se tiver usando pra outra coisa.
...
function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, 'sim') and talkState[talkUser] == 1) then
		...
		doPlayerPopupFYI(cid, "[ Tobi ] estou descansando, espere ".. getPlayerStorageValue(cid, t.storage) - os.time() .." segundos para batalhar comigo novamente.")
    
	end
	talkState[talkUser] = 0
	
	elseif(msgcontains(msg, 'ajudar')) then
	selfSay('Me ajude em minha missão da Akatsuki ! Preciso entregar 400 Okami Talisman ao meu Líder, Pode conquista-los para mim ? {entregar}', cid)
	talkState[talkUser] = 2
	elseif(msgcontains(msg, 'entregar') and talkState[talkUser] == 2) then
	if getPlayerLevel(cid) >= 60 then
	if(getPlayerItemCount(cid, 8928) >= 400 and getPlayerStorageValue(cid, 15522) < 1 ) then
	doPlayerRemoveItem(cid, 8928, 400)
	doPlayerAddExp(cid, 171200)
	doPlayerAddItem(cid, 8855, 1)
	doPlayerPopupFYI(cid, "Parabéns, você ganhou um novo item !")
	setPlayerStorageValue(cid, 15522, 1)
	...
	selfSay('Você precisa de level 60 ou mais para realizar essa missão.', cid)
	end
	talkState[talkUser] = 0
