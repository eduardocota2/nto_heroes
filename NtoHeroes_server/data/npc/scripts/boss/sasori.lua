...
local nome = "Sasori" -- nome do monstro que vai parecer

local t = {
storage = 671009,  -- storage, so mude se tiver usando pra outra coisa.
...
function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, 'sim') and talkState[talkUser] == 1) then
		...
		doPlayerPopupFYI(cid, "[ Sasori ] estou descansando, espere ".. getPlayerStorageValue(cid, t.storage) - os.time() .." segundos para batalhar comigo novamente.")
    
	end
	talkState[talkUser] = 0
	
	elseif(msgcontains(msg, 'ajudar')) then
	selfSay('Me ajude em minha missão da Akatsuki ! Preciso entregar 200 Broches de Samurais ao meu Líder, Pode conquista-los para mim ? {entregar}', cid)
	talkState[talkUser] = 2
	elseif(msgcontains(msg, 'entregar') and talkState[talkUser] == 2) then
	if getPlayerLevel(cid) >= 55 then
	if(getPlayerItemCount(cid, 8924) >= 200 and getPlayerStorageValue(cid, 12011) < 1 ) then
	doPlayerRemoveItem(cid, 8924, 200)
	doPlayerAddExp(cid, 143200)
	doPlayerAddItem(cid, 11215, 1)
	...
	selfSay('Você precisa de level 55 ou mais para realizar essa missão.', cid)
	end
	talkState[talkUser] = 0
