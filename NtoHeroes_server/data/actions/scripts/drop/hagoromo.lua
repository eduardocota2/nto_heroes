...
local monster = "Hagoromo" -- "Monstro" entre aspas
local pos = {x=1318, y=1377, z=9} -- Posição que irá nascer.
...
function onUse(cid, item, frompos, itemex, topos)
	if getStorage(2313) - os.time() < 1 then
		if CheckArea({from,to}) == 0 then
			doSetStorage(2313, os.time() + time)
		...
		doPlayerSendCancel(cid, "Voce precisa esperar ".. getStorage(2313) - os.time() .." Segundos restantes")
		doSendMagicEffect(getThingPos(cid), CONST_ME_POFF) return true
	end	
	return true
end