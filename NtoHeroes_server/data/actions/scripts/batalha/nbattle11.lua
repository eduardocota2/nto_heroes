...
local monster = "Kaguya Battle" -- "Monstro" entre aspas
local pos = {x=1821, y=706, z=9} -- Posição que irá nascer.
local from,to = {x=1808, y=697, z=9},{x=1845, y=718, z=9} -- area total da area onde o monstro vai estar
...
function onUse(cid, item, frompos, itemex, topos)
	if getStorage(5153) - os.time() < 1 then
		if CheckArea({from,to}) == 0 then
			doSetStorage(5153, os.time() + time)
		...
		doPlayerSendCancel(cid, "Voce precisa esperar ".. getStorage(5153) - os.time() .." Segundos restantes")
		doSendMagicEffect(getThingPos(cid), CONST_ME_POFF) return true
	end	
