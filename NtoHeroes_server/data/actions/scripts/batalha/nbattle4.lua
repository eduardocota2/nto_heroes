...
local monster = "Lee Battle" -- "Monstro" entre aspas
local pos = {x=1752, y=617, z=9} -- Posição que irá nascer.
local from,to = {x=1739, y=608, z=9},{x=1774, y=627, z=9} -- area total da area onde o monstro vai estar
...
function onUse(cid, item, frompos, itemex, topos)
	if getStorage(3114) - os.time() < 1 then
		if CheckArea({from,to}) == 0 then
			doSetStorage(3114, os.time() + time)
		...
		doPlayerSendCancel(cid, "Voce precisa esperar ".. getStorage(3114) - os.time() .." Segundos restantes")
		doSendMagicEffect(getThingPos(cid), CONST_ME_POFF) return true
	end	
