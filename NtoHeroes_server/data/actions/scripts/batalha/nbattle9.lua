...
local monster = "Itachi Battle" -- "Monstro" entre aspas
local pos = {x=1886, y=665, z=9} -- Posição que irá nascer.
local from,to = {x=1873, y=655, z=9},{x=1908, y=676, z=9} -- area total da area onde o monstro vai estar
...
function onUse(cid, item, frompos, itemex, topos)
	if getStorage(6239) - os.time() < 1 then
		...
		doPlayerSendCancel(cid, "Voce precisa esperar ".. getStorage(6239) - os.time() .." Segundos restantes")
		doSendMagicEffect(getThingPos(cid), CONST_ME_POFF) return true
	end	
