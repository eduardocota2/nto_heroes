...
doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "[Akat Vs Anbu] Voce esta Morto !")
setPlayerStorageValue(cid, MyTeam, -1)
doRemoveCondition(cid, CONDITION_OUTFIT)
if getGlobalStorageValue(MyTeam) == 0 then
getWinnersBattle(EnemyTeam)
else
doBroadCastBattle(23,"[Akat Vs Anbu Informacao] ".._Lib_Battle_Info.TeamOne.name.." "..getGlobalStorageValue(_Lib_Battle_Info.TeamOne.storage).." VS "..getGlobalStorageValue(_Lib_Battle_Info.TeamTwo.storage).." " .._Lib_Battle_Info.TeamTwo.name)
end
end
return true
end