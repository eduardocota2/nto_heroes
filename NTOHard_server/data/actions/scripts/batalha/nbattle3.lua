...
local monster = "Neji Battle" -- "Monstro" entre aspas
local pos = {x=1880, y=572, z=9} -- Posição que irá nascer.
local from,to = {x=1867, y=562, z=9},{x=1901, y=582, z=9} -- area total da area onde o monstro vai estar
...
function onUse(cid, item, frompos, itemex, topos)
	if getStorage(6232) - os.time() < 1 then
		if CheckArea({from,to}) == 0 then
			doSetStorage(6232, os.time() + time)
		...
		doPlayerSendCancel(cid, "Voce precisa esperar ".. getStorage(6232) - os.time() .." Segundos restantes")
		doSendMagicEffect(getThingPos(cid), CONST_ME_POFF) return true
	end	
