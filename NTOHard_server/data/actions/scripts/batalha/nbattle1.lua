...
local monster = "Naruto Battle" -- "Monstro" entre aspas
local pos = {x=1762, y=572, z=9} -- Posição que irá nascer.
local from,to = {x=1744, y=565, z=9},{x=1775, y=581, z=9} -- area total da area onde o monstro vai estar
...
function onUse(cid, item, frompos, itemex, topos)
	if getStorage(2161) - os.time() < 1 then
		if CheckArea({from,to}) == 0 then
			doSetStorage(2161, os.time() + time)
		...
		doPlayerSendCancel(cid, "Voce precisa esperar ".. getStorage(2161) - os.time() .." Segundos restantes")
		doSendMagicEffect(getThingPos(cid), CONST_ME_POFF) return true
	end	
