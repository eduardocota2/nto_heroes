...
local monster = "Kisame Battle" -- "Monstro" entre aspas
local pos = {x=1754, y=664, z=9} -- Posição que irá nascer.
local from,to = {x=1741, y=655, z=9},{x=1777, y=676, z=9} -- area total da area onde o monstro vai estar
...
function onUse(cid, item, frompos, itemex, topos)
	if getStorage(6237) - os.time() < 1 then
		if CheckArea({from,to}) == 0 then
			doSetStorage(2117, os.time() + time)
		...
		doPlayerSendCancel(cid, "Voce precisa esperar ".. getStorage(6237) - os.time() .." Segundos restantes")
		doSendMagicEffect(getThingPos(cid), CONST_ME_POFF) return true
	end	
