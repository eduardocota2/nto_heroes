...
local monster = "Rikkudo Battle" -- "Monstro" entre aspas
local pos = {x=1887, y=708, z=9} -- Posição que irá nascer.
local from,to = {x=1873, y=698, z=9},{x=1911, y=720, z=9} -- area total da area onde o monstro vai estar
...
function onUse(cid, item, frompos, itemex, topos)
	if getStorage(5553) - os.time() < 1 then
		if CheckArea({from,to}) == 0 then
			doSetStorage(5553, os.time() + time)
		...
		doPlayerSendCancel(cid, "Voce precisa esperar ".. getStorage(5553) - os.time() .." Segundos restantes")
		doSendMagicEffect(getThingPos(cid), CONST_ME_POFF) return true
	end	
