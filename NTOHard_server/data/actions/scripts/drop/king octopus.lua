...
local pos = {x=846, y=1578, z=7} -- Posição que irá nascer.

function onUse(cid, item, frompos, itemex, topos)
	if getStorage(28314) - os.time() < 1 then
		
			doSetStorage(28314, os.time() + time)
			addEvent(function() doCreateMonster(monster, pos) end, 5000)
			doCreatureSay(cid, "Prepare-se !", TALKTYPE_MONSTER)
		
	else
		doPlayerSendCancel(cid, "Voce precisa esperar ".. getStorage(28314) - os.time() .." Segundos restantes")
