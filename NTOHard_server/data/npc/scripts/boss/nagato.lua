...
local nome = "Nagato" -- nome do monstro que vai parecer

local t = {
storage = 671011,  -- storage, so mude se tiver usando pra outra coisa.
...
function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, 'sim') and talkState[talkUser] == 1) then
		...
		doPlayerPopupFYI(cid, "[ Nagato ] estou descansando, espere ".. getPlayerStorageValue(cid, t.storage) - os.time() .." segundos para batalhar comigo novamente.")
    
	end
	talkState[talkUser] = 0
	
	elseif(msgcontains(msg, 'ajudar')) then
	selfSay('Me ajude em minha missão da Akatsuki ! Preciso entregar 400 Wing of Waspoid ao meu Líder, Pode conquista-los para mim ? {entregar}', cid)
	talkState[talkUser] = 2
	elseif(msgcontains(msg, 'entregar') and talkState[talkUser] == 2) then
	if getPlayerLevel(cid) >= 65 then
	if(getPlayerItemCount(cid, 8925) >= 400 and getPlayerStorageValue(cid, 12012) < 1 ) then
	doPlayerRemoveItem(cid, 8925, 400)
	doPlayerAddExp(cid, 201700)
	...
	selfSay('Você precisa de level 65 ou mais para realizar essa missão.', cid)
	end
	talkState[talkUser] = 0
