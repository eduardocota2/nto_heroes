...
domodlib('task_func')
local nome = "Danzou" -- nome do monstro que vai parecer

local t = {
storage = 671013,  -- storage, so mude se tiver usando pra outra coisa.
...
function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, 'sim') and talkState[talkUser] == 1) then
		...
		doPlayerPopupFYI(cid, "[ Danzou ] estou descansando, espere ".. getPlayerStorageValue(cid, t.storage) - os.time() .." segundos para batalhar comigo novamente.")
    
	end
	talkState[talkUser] = 0
	
	elseif(msgcontains(msg, 'ajudar')) then
	selfSay('Você é um ninja sem emoções ? Quer dominar um poderoso genjutsu, Izanagi ? Prove-me que é capaz, traga-me 40 Glorious Heart. {entregar}', cid)
	talkState[talkUser] = 2
	elseif(msgcontains(msg, 'entregar') and talkState[talkUser] == 2) then
	if getPlayerLevel(cid) >= 75 then
	if(getPlayerItemCount(cid, 2520) >= 40 and getPlayerStorageValue(cid, 10812) < 1 ) then
	doPlayerRemoveItem(cid, 2520, 40)
	doPlayerAddExp(cid, 270200)
	doPlayerAddItem(cid, 11234, 1)
	setPlayerStorageValue(cid, 10812, 1)
	...
	selfSay('Você precisa de level 75 ou mais para realizar essa missão.', cid)
	end
	end
	return true
end
