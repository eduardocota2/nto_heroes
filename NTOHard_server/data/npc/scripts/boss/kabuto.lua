...
local nome = "Kabuto" -- nome do monstro que vai parecer

local t = {
storage = 671008,  -- storage, so mude se tiver usando pra outra coisa.
...
function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, 'sim') and talkState[talkUser] == 1) then
		...
		doPlayerPopupFYI(cid, "[ Kabuto ] estou descansando, espere ".. getPlayerStorageValue(cid, t.storage) - os.time() .." segundos para batalhar comigo novamente.")
    
	end
	talkState[talkUser] = 0
	
	elseif(msgcontains(msg, 'ajudar')) then
	selfSay('Preciso de um Chakra Especial para produzir um Jutsu Secreto ! Batalhe com Ginkaku e Kinkaku e traga-me 200 Essence of Chakra ! {entregar}', cid)
	talkState[talkUser] = 2
	elseif(msgcontains(msg, 'entregar') and talkState[talkUser] == 2) then
	if getPlayerLevel(cid) >= 50 then
	if(getPlayerItemCount(cid, 8923) >= 200 and getPlayerStorageValue(cid, 12010) < 1 ) then
	doPlayerRemoveItem(cid, 8923, 200)
	doPlayerAddExp(cid, 117700)
	doPlayerAddItem(cid, 118, 1)
	...
	selfSay('Você precisa de level 50 ou mais para realizar essa missão.', cid)
	end
	talkState[talkUser] = 0
