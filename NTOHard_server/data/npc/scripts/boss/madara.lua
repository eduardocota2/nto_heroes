...
domodlib('task_func')
local nome = "Madara" -- nome do monstro que vai parecer

local t = {
storage = 671016,  -- storage, so mude se tiver usando pra outra coisa.
...
function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, 'sim') and talkState[talkUser] == 1) then
		...
		doPlayerPopupFYI(cid, "[ Madara ] estou descansando, espere ".. getPlayerStorageValue(cid, t.storage) - os.time() .." segundos para batalhar comigo novamente.")
	...
	selfSay('Preciso de 6000 White Essence, consegue me ajudar ? {entregar}', cid)
	talkState[talkUser] = 2
	elseif(msgcontains(msg, 'entregar') and talkState[talkUser] == 2) then
	if getPlayerLevel(cid) >= 90 then
	if(getPlayerItemCount(cid, 11328) >= 6000 and getPlayerStorageValue(cid, 22542) < 1 ) then
	doPlayerRemoveItem(cid, 11328, 6000)
	doPlayerAddExp(cid, 391700)
	doPlayerAddItem(cid, 11321, 1)
	doPlayerPopupFYI(cid, "Parabéns, você ganhou um novo item !")
	setPlayerStorageValue(cid, task_sys_storages[2], (getTaskPoints(cid)+1))
	setPlayerStorageValue(cid, 22542, 1)
	selfSay('Obrigado.', cid)
	else
	selfSay('Voce precisa estudar a minha proposta, não tenho nada para você agora.', cid)
	end
	else
	selfSay('Você precisa de level 90 ou mais para realizar essa missão.', cid)
	end
	talkState[talkUser] = 0
