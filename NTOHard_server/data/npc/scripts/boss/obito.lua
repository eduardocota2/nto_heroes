...
domodlib('task_func')
local nome = "Obito" -- nome do monstro que vai parecer

local t = {
storage = 671314,  -- storage, so mude se tiver usando pra outra coisa.
...
function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, 'sim') and talkState[talkUser] == 1) then
		...
		doPlayerPopupFYI(cid, "[ Obito ] estou descansando, espere ".. getPlayerStorageValue(cid, t.storage) - os.time() .." segundos para batalhar comigo novamente.")
    
	end
	talkState[talkUser] = 0
	
	elseif(msgcontains(msg, 'ajudar')) then
		selfSay('Preciso de 16000 Purple Essence, consegue me ajudar ? {entregar}', cid)
		talkState[talkUser] = 2
		elseif(msgcontains(msg, 'entregar') and talkState[talkUser] == 2) then
		if getPlayerLevel(cid) >= 80 then
		if(getPlayerItemCount(cid, 11327) >= 16000 and getPlayerStorageValue(cid, 15512) < 1 ) then
		doPlayerRemoveItem(cid, 11327, 16000)
		doPlayerAddExp(cid, 308200)
		doPlayerAddItem(cid, 11338, 1)
		doPlayerPopupFYI(cid, "Parabéns, você ganhou um novo item !")
		setPlayerStorageValue(cid, task_sys_storages[2], (getTaskPoints(cid)+1))
		setPlayerStorageValue(cid, 15512, 1)
		selfSay('Obrigado.', cid)
	else
		selfSay('Voce precisa estudar a minha proposta, não tenho nada para você agora.', cid)
	end
	else
		selfSay('Você precisa de level 80 ou mais para realizar essa missão.', cid)
	end
	talkState[talkUser] = 0
