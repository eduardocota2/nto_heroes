...
void Player::sendCancelMessage(ReturnValue message) const
{
	switch(message)
	{
		...
		case RET_YOUNEEDTOLEARNTHISSPELL:
			sendCancel("You need to learn this jutsu first.");
			break;
		...
		case RET_YOUMAYNOTCASTAREAONBLACKSKULL:
			sendCancel("You may not cast area jutsus while you have a black skull.");
			break;
...
void Player::addExperience(uint64_t exp)
{
	...
	if(prevLevel != level)
	{
		...
		std::stringstream s;
		s << "Voc� avan�ou do Level " << prevLevel << " para o Level " << level << ".";
...
void Player::removeExperience(uint64_t exp, bool updateStats/* = true*/)
{
	...
	if(prevLevel != level)
	{
		...
		std::stringstream s;
		s << "Voc� regrediu do Level " << prevLevel << " para o Level " << level << ".";
...
bool Player::addOfflineTrainingTries(skills_t skill, int32_t tries)
{
	...
	if(skill == SKILL__MAGLEVEL)
	{
		...
		ss << "Voc� upou Ninjutsu para Level [" << magLevel << "].";
